...
migrationDataPromise.then(data => {
	...
	//force layout: transform array, only work with data
	const simulation = d3.forceSimulation();

	//definde some forces
	const forceX = d3.forceX().x( d=>w/2);
	const forceY = d3.forceY().y(h/2);
	const forceCollide = d3.forceCollide().radius(8)//define boudary
	...
	simulation
		...
		.nodes(nodesData)//which array to simulate, and start the simulation 
		.on("tick", () => {
			nodes.merge(nodesEnter)
				.attr("transform", d => `translate(${d.x},${d.y})` )
...
function generateNetwork(data, year){
	...
	filterData.forEach(od => {  //od == origin_dest
		
		//connecting the link and nodes
		const newLink = {
			value: od.value,
			source:null,
			target:null
...
console.log(
	...
	);// return a array of list of countries
	//return nodesData, linksData

console.log(linksData);

	return{
		nodesData: Array.from(nodesData.values()),//turn map to array
		linksData:linksData
	}
}
